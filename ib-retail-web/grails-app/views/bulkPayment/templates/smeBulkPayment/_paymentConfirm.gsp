...
<%--<h3>${(PaymentButtonEnum.BULKPAY.toString().equals(postProcessModel?.buttonEvent)) ? 'Pay Now' : (PaymentButtonEnum.LATER.toString().equals(postProcessModel?.buttonEvent)) ? 'Pay Later' : 'Repeat'}</h3>
	...
	--%><ul class="payment_dtls">
        ...
        <li>
         <div class="dtl_wralp">
        	...
        	<div class="rht_dtl">
        		<p><span>${postProcessModel?.fileName}</span></p>
        	</div>
        </div>	
        </li>
 	...
 	<g:if test="${!postProcessModel?.chargeCurrency.equals(null)}">
        ...
        <g:if test="${PaymentButtonEnum.PAYNOW.toString().equals(postProcessModel?.buttonEvent)}">
         <li><p class="hdr">Intended Bulk Transfer Amount</p></li>
         <li>
        	 <div class="amt_dtl">
                <p>Currency &amp; Amount</p>
    ...
    <div id="dynamicAuthContent">
		...
		<g:submitToRemote action="uploadBulkPaymentFile" controller="bulkPayment"
			...
			id="tranXConfirm" />
		<%--<input type="button" value="Cancel" class="btn_next" id="canceltrans"  onclick="postUrl('frmPayment','/ib-retail-web/bulkPayment/smeBulkPayment?tenantServiceCode=${postProcessModel?.tenantServiceCode}','canvas');closefolder();" />
