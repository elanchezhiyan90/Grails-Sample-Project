...
/************ sorting buttons *************/
	 $( ".ascending")
        ...
        icons: { primary: "ui-icon-triangle-1-n"}
	 	...
	 	.click(function() {
	 	<g:remoteFunction controller="prepaidCard" action="detailedstatementgotopage" update="mainContent" before='setSortOrder("asc")' params="\'sortOrder=\' + getSortOrder()+\'&sortBy=\'+getSortBy()+\'&gotoPage=\'+getPageNumber()"></g:remoteFunction>
<%--               ${remoteFunction(action:'detailedstatementgotopage', controller:'prepaidCard', update:'mainContent', before:'setSortOrder("asc")' ,params:'\'sortOrder=\' + getSortOrder()+\'&sortBy=\'+getSortBy()+\'&gotoPage=\'+getPageNumber()')}--%>
        })
	 .next()
	 .button({
        text: false,
        icons: { primary: "ui-icon-triangle-1-s"}
			...
			<g:remoteFunction controller="prepaidCard" action="detailedstatementgotopage" update="mainContent" before='setSortOrder("desc")'  params="\'sortOrder=\' + getSortOrder()+\'&sortBy=\'+getSortBy()+\'&gotoPage=\'+getPageNumber()"></g:remoteFunction>
			}).parent()
                    .buttonset();
