...
{
   var txt = $("#tenantServiceCodeVal").val(); 					   			 
   return txt;
...
function deleteRecord(id,refTag,bulkPayType){
		...
		var x = "Are you sure you want to take this action";
		    ...
		    $('<div>' + x + '</div>').dialog({
		        ...
		        buttons: {
		            "Yes": function() {						            	
		                $(this).dialog("close");		       
		                <vayana:deleteFunction controller="bulkPayment" action="discardBulkPaymentTransaction" params="\'pendingTransactionId=\'+id+\'&referenceTag=\'+refTag+\'&tenantServiceCode=\'+bulkPayType"  onSuccess="onDiscardSuccess(data,textStatus)" />					                
		            },
		            Cancel: function() {
...
function deleteStatusRecord(id,refTag,bulkPayType){
		...
		var x = "Are you sure you want to take this action";
		    ...
		    $('<div>' + x + '</div>').dialog({
		        ...
		        buttons: {
		            "Yes": function() {						            	
		                $(this).dialog("close");		       
		                <vayana:deleteFunction controller="bulkPayment" action="deleteBulkPaymentTransaction" params="\'pendingTransactionId=\'+id+\'&referenceTag=\'+refTag+\'&tenantServiceCode=\'+bulkPayType" update="pendingTransactionList" onSuccess="onDeleteSuccess(data,textStatus)" />					                
		            },
		            Cancel: function() {
		                $(this).dialog("close"); //close confirmation
		            }
...
function onDeleteSuccess(data,textStatus){
	$("#pendingTransactionList").html(data);
	$("#pendingTransactionList").dynamicfieldupdate();
	 setTimeout(reinitialiseScrollPane, 500); 
}
