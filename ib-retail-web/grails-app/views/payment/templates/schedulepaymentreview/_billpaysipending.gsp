...
<div class="buttons" id="confirmDiv">
						  <g:submitToRemote value="Stop Series" id="seriesButton" update="siresult" url="[controller:'payment',action:'stopSIPaymentSeries']" onSuccess="onSkipSuccess(data,textStatus);"/>
						  <g:submitToRemote value="Skip" class="btn_next" id="skipButton" update="siresult" url="[controller:'payment',action:'skipSIPayment']" onSuccess="onSkipSuccess(data,textStatus);"/>                         
</div>
</g:if>
<div id="siresult"></div>
 <g:if test="${!siViewAndUpdateModel?.paymentScheduleDetails?.isEmpty()}">
