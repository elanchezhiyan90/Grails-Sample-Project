...
<%--		<td width="25%"></td>--%>
	...
	<g:if test="${banks}">
		<g:each in="${banks}">
			<g:if test="${bankType == 'DOMESTIC'}">
				<tr class="searchResult">
					<td><input type="radio" class="loadBankValue"
						...
						data-bankname="${it.bankName}" 
						data-branchname="${it.branchName}"
			...
			<g:elseif test="${bankType == 'TENANT'}">
				<tr class="searchResult">
					<td><input type="radio" class="loadBankValue"
						name="loadBankValue" id="${it?.code}" value="${it?.code}"
						data-bankcode="${it?.code}" data-bankname="${it?.description}"
						data-branchname="${it?.description}" data-bankaddress="${it?.description}"
						data-bankidversion="${it.idVersion}" /></td>
					<td><p>
							<label for="bankCode"><span class="bankCode"> ${it?.code}
							</span></label>
						</p></td>
					<td><p>
							<label for="bankName"><span class="bankName"> ${it?.description}
							</span></label>
						</p></td>
					<td><p>
							<label for="branchName"><span class="branchName">
